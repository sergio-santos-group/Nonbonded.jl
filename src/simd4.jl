...
# """
# """
# function simd4_no_verlet_AoS(state::State{T}, cutoff::T, ::Type{Val{DoF}}) where {DoF, T <: AbstractFloat}
#     coords = view(state.coords, :)
#     energy = 0
#     for i in 1:3:length(coords)
#         vi = vload(Vec{4,Float32}, coords, i)
#         for j in (i+3):3:length(coords)
#             d = (vload(Vec{4,Float32}, coords, j) - vi)
#             d = sum(d * d *Vec{4, Float32}((1, 1, 1, 0)))
...
"""
"""
@generated function simd4_no_verlet_AoS(state::State{T}, cutoff::T, ::Type{Val{DoF}}) where {DoF, T <: AbstractFloat}
    quote
        ...
        σ = T(1)                            # LJ sigma
        mask = Vec{4, Float32}((1, 1, 1, 0))
        
        @inbounds for i = 1:3:natoms
            ...
            @inbounds for j in (i+3):3:natoms
...
"""
"""
@generated function simd4_verlet_AoS(state::State{T}, vlist::VerletList, ::Type{Val{Forces}}, ::Type{Vec{N,T}}) where {Forces, N, T<:AbstractFloat}
