...
# Generate states
xyz_aos   = generate_cubic_lattice(100.0, 10, LATTICE.primitive) # This is in AoS
...
state_aos = State(xyz_aos, zeros(T, 3, n_atoms))
state_soa = State(xyz_soa, zeros(T, n_atoms, 3))
...
energy = simd(state_aos, vlist, LennardJones.NOFORCES, Vec{8, T})
print("Energy: $energy\n")
...
# trial(:(simd_aos_sergio(state_aos, vlist, LennardJones.NOFORCES, Vec{8, T})), "SIMD_8 - Verlet - AoS (Sérgio)")