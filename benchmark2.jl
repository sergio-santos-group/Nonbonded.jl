...
# # xyz_aos   = collect(mapreduce(permutedims, vcat, [[y for x in 1:3] for y in 1:100])')
...
for N in NS
    ...
    println("CUDA | AoS | N: $N | # atoms: $n_atoms | # interacting atoms: $m_atoms | $(mean(b).time) ± $(std(b).time) ns")
end
...
#     println("CUDA | SoA | N: $N | # atoms: $n_atoms | # interacting atoms: $m_atoms | $(mean(b).time) ± $(std(b).time) ns")
# end